 ...
 */
const validateTokenCreateInputs = (wallets, config) => {
    try {
        // Validate wallets
        ...
        for (let i = 0; i < wallets.length; i++) {
            const isValidWallet = (0, utils_1.validateWallet)(wallets[i]);
            if (!isValidWallet) {
        ...
        const supportedPlatforms = ['pump', 'moon', 'bonk', 'cook', 'boop'];
 ...
 */
const getTokenCreateTransactions = async (config) => {
    try {
        const sdkConfig = (0, utils_1.getConfig)();
        const baseUrl = sdkConfig.apiUrl?.replace(/\/+$/, '');
        (0, utils_1.debugLog)('🔗 [DEBUG] Getting token creation transactions from:', `${baseUrl}/api/create`);
        ...
        (0, utils_1.debugError)('Error getting token creation transactions:', error);
        throw error;
    }
};
/**
 ...
 */
const createTokenSingle = async (wallets, config) => {
    try {
        ...
        // Validate inputs
        const validation = (0, exports.validateTokenCreateInputs)(wallets, config);
        if (!validation.valid) {
            return {
                success: false,
        ...
        const walletKeypairs = wallets.map(wallet => (0, utils_1.createKeypairFromPrivateKey)(wallet.privateKey));
        ...
        for (let i = 0; i < bundles.length; i++) {
            ...
            try {
                ...
                // Send the bundle
                const bundleResult = await (0, utils_1.sendBundle)(signedBundle.transactions);
 ...
 */
const createTokenBatch = async (walletConfigs) => {
    try {
        ...
        for (let i = 0; i < walletConfigs.length; i++) {
            ...
            const result = await (0, exports.createTokenSingle)(wallets, config);
