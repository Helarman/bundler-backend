 ...
 */
const getTokenSellTransactions = async (walletAddresses, tokenConfig) => {
    try {
        const config = (0, utils_1.getConfig)();
        const baseUrl = config.apiUrl?.replace(/\/+$/, '');
        (0, utils_1.debugLog)('🔗 [DEBUG] Getting token sell transactions from:', `${baseUrl}/api/tokens/sell`);
        ...
        (0, utils_1.debugError)('Error getting token sell transactions:', error);
        throw error;
    }
};
/**
 ...
 */
const sellTokenSingle = async (wallet, tokenConfig) => {
    try {
        ...
        // Validate inputs
        const validation = (0, exports.validateTokenSellInputs)([wallet], tokenConfig);
        if (!validation.valid) {
            return {
                success: false,
        ...
        const partiallyPreparedBundles = await getTokenSellTransactions([walletAddress], tokenConfig);
 ...
 */
const sellTokenBatch = async (wallets, tokenConfig, customPercentages) => {
    try {
        ...
        // Validate inputs
        const validation = (0, exports.validateTokenSellInputs)(wallets, tokenConfig);
        if (!validation.valid) {
            return {
                success: false,
                error: validation.error
        ...
        if (customPercentages && customPercentages.length > 0) {
            ...
            for (let i = 0; i < walletAddresses.length; i++) {
                ...
                const walletBundles = await getTokenSellTransactions([walletAddresses[i]], customConfig);
                partiallyPreparedBundles.push(...walletBundles);
            }
        }
        else {
            // Use uniform percentage for all wallets
            partiallyPreparedBundles = await getTokenSellTransactions(walletAddresses, tokenConfig);
        }
        console.log(`📦 Received ${partiallyPreparedBundles.length} bundles from backend`);
        // Step 2: Complete transaction signing
 ...
 */
const validateTokenSellInputs = (wallets, tokenConfig) => {
...
//# sourceMappingURL=tokenSell.js.map